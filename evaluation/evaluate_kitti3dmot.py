...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="./evaluation", max_truncation = 0, min_height = 25, max_occlusion = 2, cls="car", eval_3diou=True, eval_2diou=False):
    ...
    def _loadData(self, root_dir, cls, min_score=-1000, loading_groundtruth=False):
        ...
        for seq, s_name in enumerate(self.sequence_name):
            ...
            for line in f:
                ...
                t_data.ry          = float(fields[16])         # ry angle [rad]
...
def evaluate(result_sha,eval_3diou,eval_2diou):
    """
    ...
    if eval_3diou:
        print("Processing Result for KITTI 3D MOT Benchmark")
    elif eval_2diou:
        print("Processing Result for KITTI 2D MOT Benchmark")
    else:
        assert False, 'error'
    classes = []
    for c in ("car", "pedestrian", "cyclist"):
    # for c in ("car"):
        e = trackingEvaluation(t_sha=result_sha, cls=c,eval_3diou=eval_3diou,eval_2diou=eval_2diou)
        ...
        if len(e.groundtruth) != len(e.tracker):
            print("The uploaded data does not provide results for every sequence: %d vs %d" % (len(e.groundtruth), len(e.tracker)))
            return False
...
if __name__ == "__main__":
    ...
    # evaluate results
    success = evaluate(result_sha,eval_3diou,eval_2diou)
