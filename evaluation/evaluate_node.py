    ...
    def publish_labels(self):
        if not self.current_file_objects:
            if not self._load_next_file():
                ...
                try:
                    workspace_dir = '/home/vlad/workspace/tracker_prediction'
                    eval_script = os.path.join(workspace_dir, 'evaluation', 'evaluate_kitti3dmot.py')
                    
                    self.get_logger().info('Running evaluation script...')
                    cmd = f'cd {workspace_dir} && python3 {eval_script} tracker_results 3D'
                    result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
                    
                    if result.stdout:
                        self.get_logger().info('Evaluation output:\n%s' % result.stdout)
                    if result.stderr:
                        self.get_logger().error('Evaluation errors:\n%s' % result.stderr)
                        
                except Exception as e:
                    self.get_logger().error('Failed to run evaluation script: %s' % str(e))
